...
class R_dashboard extends CI_Controller
{
  ...
  //new function
  public function get_can_details()
  {
    $can_id = $this->input->post('can_id');
    $data = $this->Candidate_model->get_can_by_id($can_id);
    echo json_encode($data[0]);
  ...
  public function get_rec_details()
  {
    $rec_username = $this->session->userdata('username');
    $data = $this->profile_record_model->check_rec_details($rec_username);
    echo json_encode($data[0]);
  }

  public function Rreports_view()
  {
    $data['uname'] = $this->session->userdata('username');
    // Get report data
    $data['job_stats'] = $this->Candidate_model->get_job_stats();
    $data['interview_stats'] = $this->Calendar_model->get_interview_stats();
    $this->load->view('Recruiter_dashboard_view/Rreports', $data);
  }

  public function Areports_view()
  {
      $data['uname'] = $this->session->userdata('username');
      // Get admin-level report data
      $data['job_stats'] = $this->Candidate_model->get_all_job_stats();
      $data['interview_stats'] = $this->Calendar_model->get_all_interview_stats();
      $data['recruiter_stats'] = $this->Signup_model->get_recruiter_stats();
